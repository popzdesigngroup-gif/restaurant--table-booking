...
export const Navbar: React.FC = () => {
  ...
  const links = isAdminUser ? adminLinks : customerLinks;
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-slate-950/80 backdrop-blur-xl border-b border-slate-800/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        {/* Brand Logo */}
        <Link href={isAdminUser ? '/admin/dashboard' : '/'} className="flex items-center gap-2.5 group">
          ...
          <div>
            <span className="text-lg font-black tracking-tight text-white flex items-center gap-1">
              ...
              {isAdminUser && (
                <span className="text-[10px] bg-indigo-500/20 text-indigo-400 px-2 py-0.5 rounded-md border border-indigo-500/30 font-bold ml-1">
                  ADMIN PORTAL
                </span>
              )}
            </span>
        ...
        {/* Navigation Links Driven strictly by Login Role */}
        ...
        {/* User Details & Login Credentials Controls */}
              ...
              <div className="text-right hidden sm:block">
                ...
                <div
                  className={`text-[10px] font-semibold capitalize mt-0.5 ${
                    isAdminUser ? 'text-indigo-400' : 'text-emerald-400'
                  }`}
                >
                  {user.role} Account
                </div>
